...
import { getProductDetail } from "../queries/product-detail.js";
import {
  searchProducts,
  listProductGroups,
...
// Registered after the literal paths above, so "groups" is not read as an id.
products.get("/:id", (c) => {
  const id = Number(c.req.param("id"));
  if (!Number.isInteger(id)) return c.json({ error: "id must be an integer" }, 400);
  const branch = c.req.query("branch");

  const result = getProductDetail(id, branch ? Number(branch) : null);
